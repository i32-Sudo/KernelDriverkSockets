...
MemoryIO::MemoryIO(PMem& physmem_api, SOCKET connection) :physmem_api(physmem_api)
{
	...
	cached_dwPID = find_process_by_id(memConfig::processExe);
...
uint64_t MemoryIO::find_process_by_id(const std::string& name)
{
...
uint64_t PMem::GetModuleBaseAddress(SOCKET connection = memio->getConnection(), uint32_t process_id = memio->getCachedPID(), const char* moduleName = NULL) {
	if (moduleName == NULL)
		return 0;
	...
	packet.header.magic = packet_magic;
	packet.header.type = PacketType::packet_get_module;

	auto& data = packet.data.get_module_base;
	data.process_id = process_id;
	strcpy(data.moduleName, moduleName);
