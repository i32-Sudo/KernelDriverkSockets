...
public:
	...
	uint64_t GetModuleBaseAddress(SOCKET connection, uint32_t process_id, const char* moduleName);
};
...
public:
	...
	uint64_t find_process_by_id(const std::string& name);

	SOCKET getConnection() const { return cached_connection; };
	int getCachedPID() const { return cached_dwPID; };
private:
