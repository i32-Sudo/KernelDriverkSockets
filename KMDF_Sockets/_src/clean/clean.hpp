...
PVOID GetKernelBase(OUT PULONG pSize)
{
	...
	if (bytes == 0)
	{
		log(skCrypt("Invalid SystemModuleInformation size"));
		return NULL;
	}
...
extern "C" bool LocatePiDDB(PERESOURCE * lock, PRTL_AVL_TABLE * table)
{
	...
	if (NT_SUCCESS(BBScanSection(skCrypt("PAGE"), PiDDBLockPtr_sig_win10, 0, sizeof(PiDDBLockPtr_sig_win10) - 1, reinterpret_cast<PVOID*>(&PiDDBLockPtr)))) {
		/* Win10 Signature Captured */
		PiDDBLockPtr = PVOID((uintptr_t)PiDDBLockPtr + 28);
		log(skCrypt("Win10 Signature Found"));
	}
	else {
		/* Win10 Signature Failed */
		if (NT_SUCCESS(BBScanSection(skCrypt("PAGE"), PiDDBLockPtr_sig_win11, 0, sizeof(PiDDBLockPtr_sig_win11) - 1, reinterpret_cast<PVOID*>(&PiDDBLockPtr)))) {
			/* Win11 Signature Captured */
			PiDDBLockPtr = PVOID((uintptr_t)PiDDBLockPtr + 16);
			log(skCrypt("Win11 Signature Found"));
		}
		else {
			/* Both Failed */
			log(skCrypt("Could not find PiDDB for Win10 or Win11..."));
			return 1;
		}

	}

	if (!NT_SUCCESS(BBScanSection(skCrypt("PAGE"), PiDDBCacheTablePtr_sig, 0, sizeof(PiDDBCacheTablePtr_sig) - 1, reinterpret_cast<PVOID*>(&PiDDBCacheTablePtr)))) {
		log(skCrypt("Unable to find PiDDBCacheTablePtr sig"));
		return false;
	}
...
namespace clear {
	...
	BOOL clearCache(UNICODE_STRING DriverName, ULONG timeDateStamp) {
		...
		if (!RtlDeleteElementGenericTableAvl(PiDDBCacheTable, pFoundEntry)) {
			log(skCrypt("RtlDeleteElementFromTableAVL Failed!"));
			return 1;
		}

		// release the ddb resource lock
		ExReleaseResourceLite(PiDDBLock);
		log(skCrypt("chache cleared"));
		return 0;
	}

	BOOL clearHashBucket(UNICODE_STRING DriverName) {
		char* CIDLLString = skCrypt("ci.dll");
		CONST PVOID CIDLLBase = GetKernelModuleBase(CIDLLString);

		if (!CIDLLBase) {
			log(skCrypt("Couldn't Find CIDDLBase"));
			return 1;
		}

		char* pKernelBucketHashPattern_21H1 = skCrypt(KernelBucketHashPattern_21H1);
		char* pKernelBucketHashMask_21H1 = skCrypt(KernelBucketHashMask_21H1);

		char* pKernelBucketHashPattern_22H2 = skCrypt(KernelBucketHashPattern_22H2);
		char* pKernelBucketHashMask_22H2 = skCrypt(KernelBucketHashMask_22H2);
		...
		if (!SignatureAddress) {
			...
			if (!SignatureAddress) {
				log(skCrypt("Couldn't find signature address for KernelBucketHash"));
				return 1;
			}
		}
		...
		if (Status == FALSE) {
			log(skCrypt("KernelHashBucket Failed to Clean"));
			return 1;
		}
		else {
			log(skCrypt("KernelHashBucket Cleaned!"));
			return 0;
		}
		return 0;
	}
	...
	BOOL
		...
		if (ps_loaded == NULL) {
			log(skCrypt("Failed to get ps_loaded resource"));
			return 1;
		}
		...
		if (Modified == FALSE) {
			log(skCrypt("No modifications were made"));
			return 1;
		}
		else {
			log(skCrypt("Modifications to MMU/MML Were made and have been cleared..."));
			return 0;
		}
